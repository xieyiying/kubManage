<template>
    <div class="test">
        <c-breadcrumb tableTitle="可优比管理"></c-breadcrumb> 
        <div class="container">
            <c-search
              @delAll="handleDelAll"
...
<script>
    import { kubColumn } from '@/config/tableColumn'
    ...
    export default {
        ...
        data() {
            return {
                tableObject: {
                    data: [], // 表格数据
                    column: kubColumn, // 表格头
                    buttons: [ // 操作按钮
                        {
                            text: '编辑',
                            callback: (index, row) => {
                                this.$router.push({path: '/editKubMsg', query: {
                                    id: row.id,
                                    title: '编辑'
                                }})
                            },
        ...
        methods: {
            // 获取用户信息列表
            getData(currentPage) {
                kubInterfaceRequest.getListData({
                    pageNo: currentPage,
                    pageSize: 10
                }).then(res => {
                    if(res.success) {
            ...
            // 新增
            handleAddMessage() {
                this.$router.push({path: '/editKubMsg', query: {title: '新增'}})
            },
            // 批量选择删除的条目
            ...
            handleDelete() {
                delMethods(
                    this.delflag,
                    kubInterfaceRequest.deleteData,
                    {id: this.delId},
                    kubInterfaceRequest.batchDeleteData,
